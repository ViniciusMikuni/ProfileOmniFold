...
from tensorflow.keras.layers import Dense, Input, TimeDistributed, LeakyReLU, Concatenate,AveragePooling1D, Masking, Dropout
...
class Multifold():
    ...
    def RunStep1(self,i):
        ...
        self.RunModel(
            ...
            np.concatenate((self.not_pass_reco==0, np.ones_like(self.labels_data,dtype=bool))),
        ...
        if self.verbose:            
            ...
            weight_dict = {
                'mc reco':self.weights_pull[self.not_pass_reco==0]*self.weights_mc[self.not_pass_reco==0],
                'data reco': self.weights_data,
            }

            feed_dict = {
                'mc reco':self.mc_reco[self.not_pass_reco==0,0,0],
                'data reco':self.data[:,0,0],
            }
            ...
            fig.savefig('../plots/Unfolded_Hist_step1_iter{}_{}.pdf'.format(i,self.opt['NAME']))
    ...
    def RunStep2(self,i):
        ...
        self.RunModel(
            ...
            np.concatenate((self.not_pass_gen==0, self.not_pass_gen==0)),
    ...
    def RunModel(self,sample,labels,mask,weights,iteration,model,stepn):
    ...
    def PrepareWeights(self,weights_mc,weights_data,pass_reco,pass_gen):
        ...
        if self.strapn>0:
            ...
            self.weights_data = np.random.poisson(1,self.data.shape[0]) * self.weights_data
    ...
    def PrepareModel(self,nvars):
        inputs1,outputs1 = DeepSets(nvars)
        inputs2,outputs2 = DeepSets(nvars)
...
def DeepSets(nvars,nlayers = 4,mlp_dim=64):
    inputs = Input((None,nvars))
    masked_inputs = Masking(mask_value=0.0,name='Mask')(inputs)
    residual =  TimeDistributed(Dense(mlp_dim))(masked_inputs)
    layer = residual
    for _ in range(nlayers):
        layer = TimeDistributed(Dense(mlp_dim))(layer)
        layer = TimeDistributed(LeakyReLU(alpha=0.01))(layer)
        
    residual = Concatenate(-1)([residual,layer])
    residual = TimeDistributed(Dense(mlp_dim))(residual)
    residual = TimeDistributed(LeakyReLU(alpha=0.01))(residual)
    pooled = AveragePooling1D()(residual)
    
    representation =  Dense(2*mlp_dim,activation=None)(pooled)    
    representation =  LeakyReLU(alpha=0.01)(representation)
    representation = Dropout(0.2)(representation)
    representation =  Dense(mlp_dim,activation=None)(pooled)    
    representation =  LeakyReLU(alpha=0.01)(representation)
    representation = Dropout(0.2)(representation)
